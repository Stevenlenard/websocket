...
<body>
  ...
  <script>
    document.addEventListener("DOMContentLoaded", () => {
      ...
      loginForm.addEventListener("submit", async e => {
        ...
        try {
          ...
          if (data.success) {
            ...
            // Fast session warm-up: race session-init POST vs a short timeout (~700ms)
            try {
              const sessionForm = new FormData();
              sessionForm.append('email', emailInput.value || '');
              sessionForm.append('role', formData.get('role') || 'janitor');
              ...
              const sessionResult = await Promise.race([
                sessionFetch,
                new Promise(resolve => setTimeout(() => resolve({success:false, timeout:true}), 700))
              ]);
            ...
            if (data.errors && data.errors.general) showNotification(data.errors.general, "error");
          }
        } catch (err) {
      ...
      // Pre-fill email from cookie if exists (moved inside DOMContentLoaded to access emailInput safely)
